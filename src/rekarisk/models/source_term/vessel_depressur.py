...
class VesselInput:
    ...
    wall_mass: float = 0.0            # Vessel wall mass [kg]. If > 0, enables wall inertia model.
    wall_cp: float = 500.0           # Wall specific heat capacity [J/(kg·K)] (default: carbon steel).
    U_wall_gas: float = 0.0          # Wall-to-gas HTC [W/(m²·K)]. If > 0, overrides U_htc for wall→gas path.
...
class VesselResult:
    ...
    T_wall: np.ndarray | None = None   # Wall temperature array [K] (wall inertia model).
    ...
    def to_csv(
        ...
        for i in range(n):
            ...
            if self.T_wall is not None:
                row.append(f"{T_conv(self.T_wall[i]):.4f}")
            if self.Z is not None:
...
def _gas_blowdown_ode_with_wall(t: float, y: np.ndarray, params: dict) -> np.ndarray:
    """ODE right-hand side for gas blowdown WITH wall thermal inertia.

    State vector y = [m, U, U_wall] where:
        m = mass of gas in vessel [kg]
        U = total internal energy of gas [J]
        U_wall = total internal energy of vessel wall [J]

    ODEs:
        dm/dt = -mdot
        dU/dt = -mdot * h_out + Q_wall_to_gas
        dU_wall/dt = Q_amb_to_wall - Q_wall_to_gas

    Heat transfer paths:
        ambient/fire → wall (U_htc × A × (T_amb - T_wall))
        wall → gas (U_wall_gas × A × (T_wall - T_gas))
    """
    m, U, U_wall = y
    V = params["V"]
    k = params["k"]
    ...
    Cd = params["Cd"]
    A_wall = params["A_wall"]
    U_htc = params["U_htc"]      # ambient/fire → wall
    U_wg = params["U_wall_gas"]  # wall → gas
    T_amb = params["T_amb"]
    P_down = params["P_down"]
    wall_cp = params["wall_cp"]
    wall_mass = params["wall_mass"]
    ...
    if P <= P_down + EPSILON:
        # No flow, but still heat transfer
        Q_wg = U_wg * A_wall * (T_wall - T_gas) if U_wg > EPSILON else 0.0
        Q_aw = U_htc * A_wall * (T_amb - T_wall) if U_htc > EPSILON else 0.0
        return np.array([0.0, Q_wg, Q_aw - Q_wg])
    ...
    if is_choked:
        G_flux = _gas_choked_mass_flux(P, T_gas, k, MW, Z_c)
    else:
        pr = P_down / P
        ratio_term = pr ** (2.0 / k) - pr ** ((k + 1.0) / k)
        if ratio_term <= EPSILON or pr <= EPSILON:
            return np.array([0.0, 0.0, 0.0])
        factor = (2.0 * k / (k - 1.0)) * (MW / (Z_c * R * T_gas))
        G_flux = P * math.sqrt(factor * ratio_term)

    mdot = Cd * A_hole * G_flux

    # Enthalpy of exiting gas
    cp_val = cv * k
    h_out = cp_val * T_gas

    # Heat transfer: wall → gas
    Q_wg = U_wg * A_wall * (T_wall - T_gas) if U_wg > EPSILON else 0.0

    # Heat transfer: ambient/fire → wall
    Q_aw = U_htc * A_wall * (T_amb - T_wall) if U_htc > EPSILON else 0.0

    # Derivatives
    dm_dt = -mdot
    dU_dt = -mdot * h_out + Q_wg        # gas loses mass+energy, gains heat from wall
    dU_wall_dt = Q_aw - Q_wg            # wall gains from ambient, loses to gas

    return np.array([dm_dt, dU_dt, dU_wall_dt])


def _solve_gas_blowdown(inputs: VesselInput) -> dict:
    """Solve gas blowdown using ODE integration.

    Supports optional wall thermal inertia model when wall_mass > 0.
    Without wall inertia: 2-state ODE [m, U]
    With wall inertia: 3-state ODE [m, U, U_wall]
    ...
    # Detect wall inertia mode
    use_wall = inputs.wall_mass > 0.0 and inputs.A_wall > 0.0
    # U_wall_gas: use explicit value if provided, else fall back to U_htc
    U_wg = inputs.U_wall_gas if inputs.U_wall_gas > 0 else inputs.U_htc
    ...
    if use_wall:
        # ---- Wall inertia mode (3-state) ----
        params["U_wall_gas"] = U_wg
        params["wall_cp"] = inputs.wall_cp
        params["wall_mass"] = inputs.wall_mass

        U_wall0 = inputs.wall_mass * inputs.wall_cp * inputs.T_initial
        y0 = np.array([m0, U0, U_wall0])
        ode_func = _gas_blowdown_ode_with_wall

        def pressure_reached(t, y, *args):
            m, U, U_w = y
            if m <= 1e-15:
                return 0.0
            T = U / (m * cv_val)
            P = _gas_pressure(inputs.V, m, T, inputs.molecular_weight, inputs.Z)
            return P - P_target

        def wall_mass_event(t, y):
            """Prevent wall from going below absolute zero."""
            return y[2] - 1.0  # U_wall > 0
        wall_mass_event.terminal = False

        pressure_reached.terminal = True
        pressure_reached.direction = -1
        ...
        U_wall_interp = np.interp(t_eval, sol.t, sol.y[2])
        ...
        # ---- Standard mode (2-state, no wall inertia) ----
        def pressure_reached(t, y, *args):
            m, U = y
            if m <= 1e-15:
                return 0.0
